import { CalloutKeepType, type GlobalSettings } from "../interface";
...
export function replaceAllDivCalloutToBlockquote(
	...
	for (const divCallout of allDivCallout) {
		if (
			settings.callout !== CalloutKeepType.Remove &&
			settings.callout !== CalloutKeepType.RemoveKeepTitle &&
			...
			const calloutType =
				...
				(divCallout.parentElement
					? divCallout.parentElement.attributes.getNamedItem("data-callout")?.value
					: "");
	...
	for (const titleInner of allTitleInner) {
		if (titleInner && settings.callout !== CalloutKeepType.Remove) {
			titleInner.innerHTML = calloutTitle
				.toLowerCase()
				.contains(titleInner.innerHTML.toLowerCase())
